...
const formatCurrency = (amount: number): string => {
    return `${amount >= 0 ? '+' : '-'}${Math.abs(amount).toLocaleString('vi-VN')}đ`;
};
</script>
    ...
    <div class="space-y-3">
        <div class="flex flex-col gap-2 rounded-[10px] bg-slate-50/80 p-3 md:flex-row md:items-center">
            <input
                ...
                placeholder="Search mã giao dịch hoặc nội dung..."
            />
        ...
        <div class="overflow-hidden rounded-[10px] border border-slate-200/80">
            <div v-if="loading" class="bg-slate-50 px-4 py-10 text-center text-sm text-slate-500">Đang tải lịch sử dòng tiền...</div>
            ...
            <div v-else class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        <tr v-for="item in transactions" :key="item.id" class="align-top">
        ...
        <div class="flex flex-col gap-2 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
            <p>Hiển thị {{ transactions.length }} / {{ meta.total }} giao dịch</p>
