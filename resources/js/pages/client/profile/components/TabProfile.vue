...
<template>
    <div class="grid gap-3 xl:grid-cols-[minmax(0,1.2fr)_340px]">
        <form class="space-y-4 rounded-[10px] border border-teal-100 bg-white p-4" @submit.prevent="emit('submit')">
            <div class="flex flex-col gap-3 rounded-[10px] bg-teal-50/70 p-3 md:flex-row md:items-center">
                <div class="relative">
                    <div
                        class="flex h-20 w-20 items-center justify-center overflow-hidden rounded-[20px] border border-teal-100 bg-[linear-gradient(135deg,_#dcfce7_0%,_#ecfdf5_100%)]"
                    >
                    ...
                    <span class="absolute -bottom-1.5 -right-1.5 flex h-8 w-8 items-center justify-center rounded-full bg-teal-600 text-white shadow-lg">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <h2 class="mt-1 text-xl font-black tracking-[-0.04em] text-slate-900">{{ props.user?.full_name || props.user?.username || 'Người dùng' }}</h2>
            ...
            <div class="grid gap-3 md:grid-cols-2">
                ...
                <label class="space-y-1.5">
                    ...
                    <input
                        ...
                        type="text"
                        class="w-full rounded-[8px] border border-teal-100 bg-teal-50/70 px-3 py-2.5 text-sm text-slate-700 outline-none"
                ...
                <label class="space-y-1.5">
                    ...
                    <input
                        ...
                        type="text"
                        disabled
                        class="w-full rounded-[8px] border border-teal-100 bg-teal-50/70 px-3 py-2.5 text-sm text-slate-500 outline-none"
                    />
                </label>
            </div>
            ...
            <div class="flex justify-end">
                <button
                    type="submit"
                    class="inline-flex items-center justify-center rounded-[10px] bg-teal-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-500 disabled:cursor-not-allowed disabled:opacity-60"
        ...
        <section class="space-y-3">
            <div class="rounded-[10px] border border-teal-100 bg-white p-4">
                <div class="flex items-center gap-2">
                    <MailCheck class="h-4 w-4 text-teal-600" />
                    <h3 class="text-sm font-bold text-slate-900">Tổng quan xác thực</h3>
                </div>

                <div class="mt-3 space-y-2">
                    <div v-for="meta in props.accountMeta" :key="meta.label" class="flex flex-col gap-1 rounded-[10px] bg-teal-50/70 px-3 py-2.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
            ...
            <div class="rounded-[10px] border border-teal-100/80 bg-[linear-gradient(135deg,_#ecfdf5_0%,_#f0fdfa_100%)] p-4">
                <div class="flex items-center gap-2">
