...
<template>
    <div class="grid gap-3 xl:grid-cols-[minmax(0,1.2fr)_340px]">
        <form class="space-y-4 rounded-[10px] border border-slate-200/80 bg-white p-4" @submit.prevent="$emit('submit')">
            <div class="flex flex-col gap-3 rounded-[10px] bg-slate-50/80 p-3 md:flex-row md:items-center">
                <div class="relative">
                    <div
                        class="flex h-20 w-20 items-center justify-center overflow-hidden rounded-[20px] border border-slate-200 bg-[linear-gradient(135deg,_#dbeafe_0%,_#e0f2fe_100%)]"
                    >
                    ...
                    <span class="absolute -bottom-1.5 -right-1.5 flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-white shadow-lg">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <h2 class="mt-1 text-xl font-black tracking-[-0.04em] text-slate-900">{{ user?.full_name || user?.username || 'Người dùng' }}</h2>
                    <p class="mt-1 text-sm text-slate-500">{{ user?.email || 'Chưa có email' }}</p>
                </div>
            </div>
            ...
            <div class="grid gap-3 md:grid-cols-2">
                <label class="space-y-1.5">
                    ...
                    />
                    <p v-if="errors.avatar" class="text-xs text-rose-600">{{ errors.avatar }}</p>
                </label>
                ...
                <label class="space-y-1.5">
                    ...
                    />
                    <p v-if="errors.phone" class="text-xs text-rose-600">{{ errors.phone }}</p>
                </label>
            ...
            <div class="flex justify-end">
                <button
                    type="submit"
                    class="inline-flex items-center justify-center rounded-[10px] bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
                    :disabled="saving"
                >
        ...
        <section class="space-y-3">
            <div class="rounded-[10px] border border-slate-200/80 bg-white p-4">
                <div class="flex items-center gap-2">
                    <MailCheck class="h-4 w-4 text-emerald-600" />
                    <h3 class="text-sm font-bold text-slate-900">Tổng quan xác thực</h3>
                </div>

                <div class="mt-3 space-y-2">
                    <div
                        v-for="meta in accountMeta"
                        :key="meta.label"
                        class="flex items-start justify-between gap-3 rounded-[10px] bg-slate-50/80 px-3 py-2.5"
                    >
            ...
            <div class="rounded-[10px] border border-slate-200/80 bg-[linear-gradient(135deg,_#eff6ff_0%,_#f8fafc_100%)] p-4">
                <div class="flex items-center gap-2">
