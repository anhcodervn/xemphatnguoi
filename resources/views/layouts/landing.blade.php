...
<head>
    ...
    @php
        ...
        $siteDescription =
            ...
            ($settings['site_description'] ?? 'Nền tảng SaaS cho thuê và quản lý HTTP Cron Jobs với logs, alerts và quota theo gói.');
        ...
        $footerLinkGroups = [
            [
                ...
                'links' => [
                    ...
                    ['label' => 'Blog', 'href' => route('seo.index')],
            ...
            [
                ...
                'links' => [
                    ...
                    ['label' => 'Chính sách sử dụng dịch vụ', 'href' => url('/chinh-sach-su-dung-api')],
    ...
    <link href="https://fonts.bunny.net/css?family=be-vietnam-pro:400,500,600,700|space-grotesk:500,700|jetbrains-mono:400,600" rel="stylesheet">
...
<body class="bg-white text-slate-900">
    <div class="min-h-screen">
        <header data-landing-header class="sticky top-0 z-30 border-b border-slate-200/70 bg-white/88 backdrop-blur-xl transition duration-200">
            <div class="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
                <a href="/" class="shrink-0">
                    ...
                    @else
                        <div class="flex items-center gap-3">
                            <div class="flex h-10 w-10 items-center justify-center rounded-[10px] border border-sky-100 bg-sky-50 text-sm font-bold text-sky-700">
                                {{ \Illuminate\Support\Str::substr($siteName, 0, 2) }}
                            </div>
                            <div>
                                <div class="font-tech text-lg font-bold tracking-tight text-slate-950">{{ $siteName }}</div>
                                <div class="font-mono-tech text-[11px] uppercase tracking-[0.24em] text-sky-500">HTTP Cron SaaS</div>
                            </div>
                        </div>
                    @endif
                </a>
                ...
                <nav class="hidden items-center gap-8 text-[15px] font-semibold text-slate-700 md:flex">
                    @if (Auth::check())
                        ...
                        <a href="/cron-jobs" class="transition hover:text-sky-700">Cron Jobs</a>
                        <a href="/alert-channels" class="transition hover:text-sky-700">Kênh cảnh báo</a>
                        <a href="/package" class="transition hover:text-sky-700">Gói dịch vụ</a>
                        <a href="/profile" class="transition hover:text-sky-700">Tài khoản</a>
                    @else
                        <a href="{{ $landingSectionLinks['features'] }}" class="transition hover:text-sky-700">Tính năng</a>
                        <a href="{{ $landingSectionLinks['pricing'] }}" class="transition hover:text-sky-700">Bảng giá</a>
                        <a href="{{ route('seo.index') }}" class="transition hover:text-sky-700">Blog</a>
                    ...
                    <div class="absolute right-0 top-12 z-20 w-72 rounded-[14px] border border-slate-200 bg-white p-3 shadow-[0_20px_50px_rgba(15,23,42,0.12)] backdrop-blur-xl">
                        <nav class="grid gap-1 text-sm font-semibold text-slate-700">
                            @if (Auth::check())
                                ...
                                <a href="/cron-jobs" class="rounded-[10px] px-4 py-3 transition hover:bg-slate-50 hover:text-sky-700">Cron Jobs</a>
                                <a href="/alert-channels" class="rounded-[10px] px-4 py-3 transition hover:bg-slate-50 hover:text-sky-700">Kênh cảnh báo</a>
                                <a href="/package" class="rounded-[10px] px-4 py-3 transition hover:bg-slate-50 hover:text-sky-700">Gói dịch vụ</a>
                                <a href="/profile" class="rounded-[10px] px-4 py-3 transition hover:bg-slate-50 hover:text-sky-700">Tài khoản</a>
                            @else
                                ...
                                <a href="{{ $landingSectionLinks['pricing'] }}" class="rounded-[10px] px-4 py-3 transition hover:bg-slate-50 hover:text-sky-700">Bảng giá</a>
                                <a href="{{ route('seo.index') }}" class="rounded-[10px] px-4 py-3 transition hover:bg-slate-50 hover:text-sky-700">Blog</a>
            ...
            <div class="mx-auto grid w-full max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1.2fr)_repeat(2,minmax(0,1fr))] lg:px-8">
                <div class="max-w-md">
                    ...
                    <p class="mt-3 text-sm leading-7 text-slate-600">
                        {{ $settings['site_description'] ?? 'AutoCron giúp tạo, chạy và giám sát HTTP Cron Jobs theo lịch với giới hạn theo gói, logs rõ ràng và cảnh báo kịp thời.' }}
                    </p>
            ...
            <div class="border-t border-slate-200/80 bg-slate-50">
                <div class="mx-auto flex w-full max-w-7xl flex-col gap-2 px-4 py-4 text-sm text-slate-500 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        <a href="{{ url('/chinh-sach-su-dung-api') }}" class="transition hover:text-slate-900">Chính sách dịch vụ</a>
                    </div>
                </div>
            </div>
        </footer>
    ...
    @if (!empty($settings['gtm_id']))
        <noscript>
            <iframe src="https://www.googletagmanager.com/ns.html?id={{ $settings['gtm_id'] }}" height="0" width="0"
