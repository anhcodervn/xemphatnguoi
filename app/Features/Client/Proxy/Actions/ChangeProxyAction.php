...
class ChangeProxyAction
{
    ...
    private function execute(User $user, int $proxyId, array $payload, bool $synchronous): array
    {
        ...
        if ($synchronous) {
            return $this->proxyOperationService->change($user, $proxy, $payload, true);
        }

        return $this->proxyOperationService->change($user, $proxy, $payload);
    }
}
