...
class VcbCaptcha
{
    private const DEFAULT_API_URL = 'https://captcha-vcb.apibankvn.com/api/captcha/vcb';
    ...
    public function solve(string $base64): array
    {
        ...
        if ($normalizedBase64 === '') {
            throw new ApiException('Thiếu dữ liệu ảnh captcha VCB.', 422);
        }
    ...
    private function normalizeResponse(Response $response): array
    {
        ...
        if (! $response->successful()) {
            throw new ApiException('Dịch vụ giải captcha VCB tạm thời không phản hồi.', 502, [
